...
void main()
{
    ...
    for (int i=0;i<numLights;i++)
    {
        ...
        //binning for toon shading!
        float shade;
        float nDotl10 = nDotL * 10;

        shade = ceil(nDotl10) / 10;   

        ambient = material.ambient * light[i].ambient * shade;
        diffuse = material.diffuse * light[i].diffuse * max(nDotL,0) * shade;
        if (nDotL>0)
            specular = material.specular * light[i].specular * pow(rDotV,material.shininess) * shade;
        else
            specular = vec3(0,0,0);
